...
import datetime
import uuid
import streamlit as st # Keep streamlit import here
...
if not GEMINI_API_KEY:
    ...
    st.stop() # Stop execution if key is missing - NOW SAFE TO CALL
...
try:
    ...
    # You can optionally show a success message later, e.g., in the sidebar
    # st.sidebar.success(f"Gemini OK ({MODEL_NAME})") # Example
except Exception as e:
    st.error(f"🔴 Error configuring Gemini API: {e}")
    st.stop() # NOW SAFE TO CALL
# --- End Configuration ---
...
def generate_resources_from_profile(emotion_analysis: Dict, growth_plan: Optional[Dict]) -> Dict:
    ...
    # Note: Removing st.info from here to avoid potential issues if called before main loop
    # Consider adding spinner logic within the UI button calls instead.
    # st.info("Generating personalized resource suggestions with Gemini...")
    return call_gemini_llm(prompt)


# --- UI Rendering Functions (Remain the same as previous Gemini version) ---
    ...
    with col2:
        ...
        with st.form("signup_form"):
            ...
            if st.form_submit_button("Sign Up"):
                if not signup_username or not signup_password: st.warning("Username and password required.")
                elif signup_username in st.session_state.user_db: st.error("Username already taken.")
                elif len(signup_password) < 4: st.error("Password must be at least 4 characters.")
                else:
...
def render_sidebar():
    ...
    if selected_view != current_view:
        ...
        if selected_view in ["main", "journal"]:
...
def render_main_dashboard():
    ...
    with col2:
        if st.button("📝 New Journal Entry", type="primary", use_container_width=True, key="new_entry_dash_gem"):
            ...
            st.rerun()
    st.divider()
    if not emotion_history: st.info("Start your journey: add your first journal entry!")
    else:
        ...
        for entry in sorted_history[:5]:
            ...
            analysis = entry.get('analysis', {}); is_analysis_ok = isinstance(analysis, dict) and 'error' not in analysis and 'raw_response' not in analysis
            ...
            with st.expander(title):
                st.caption("Entry:"); st.write(entry.get('journal_entry', 'N/A'))
                if not is_analysis_ok: st.warning("Analysis may have issues.")
                if st.button("View Analysis & Actions", key=f"view_dash_{entry_id}"):
                    st.session_state.current_emotion_id = entry_id; st.session_state.current_view = "emotion_analysis"; st.rerun()
...
def render_journal_page():
    ...
    with st.form("journal_form_gem"):
        ...
        if st.form_submit_button("Analyze My Emotions", type="primary"):
            if not journal_entry or len(journal_entry.strip()) < 10: st.warning("Please write a bit more for analysis.")
            else:
                with st.spinner("🧠 Analyzing with Gemini..."):
                    ...
                    st.session_state.current_emotion_id = e_id; st.session_state.current_view = "emotion_analysis"; st.rerun()
    ...
    if not history: st.info("No past entries.")
        ...
        for entry in sorted_hist:
             ...
             p_emo = analysis.get('primary_emotion', 'Entry') if is_ok else 'Entry'; prev = entry.get('journal_entry', '')[:80] + '...'
...
def render_emotion_analysis(emotion_id: str):
    ...
    user_id = st.session_state.current_user; emotion_data = get_emotion_entry(user_id, emotion_id)
    if not emotion_data:
        st.error("Entry not found.");
        if st.button("Back", key="back_analysis_err"): st.session_state.current_view = "journal"; st.rerun()
        return
    st.subheader("Your Journal Entry"); st.markdown(f"> _{emotion_data.get('journal_entry', 'N/A')}_"); st.caption(f"Logged: {emotion_data.get('timestamp', '')[:19].replace('T', ' ')}")
    st.divider(); st.subheader("Gemini AI Analysis")
    analysis = emotion_data.get('analysis', {}); analysis_successful = isinstance(analysis, dict) and 'error' not in analysis and 'raw_response' not in analysis
    if not analysis: st.warning("Analysis data missing.")
        ...
        col1, col2 = st.columns(2)
        with col1: st.metric("Primary Emotion", analysis.get('primary_emotion', 'N/A').capitalize());
        if 'intensity' in analysis: st.metric("Intensity (1-10)", analysis['intensity'])
        with col2: triggers = analysis.get('triggers', []); st.markdown("**Potential Triggers:**");
        if triggers: st.write('\n'.join(f"- {t}" for t in triggers)); else: st.write("_None identified._")
        growth_opps = analysis.get('growth_opportunities', []); st.markdown("**Growth Opportunities:**");
        if growth_opps: st.write('\n'.join(f"- {g}" for g in growth_opps)); else: st.write("_None identified._")
    st.divider(); st.subheader("Next Steps"); col1, col2 = st.columns(2)
    growth_plan = get_growth_plan(user_id, emotion_id); resources = get_resources(user_id, emotion_id)
    with col1: # Growth Plan Actions
        st.markdown("##### **Personal Growth Plan**"); plan_exists = isinstance(growth_plan, dict) and 'error' not in growth_plan and 'raw_response' not in growth_plan
        if plan_exists: st.success("Growth plan generated.");
        if st.button("View Plan", key="view_plan_btn_gem", use_container_width=True, disabled=not plan_exists): st.session_state.current_view = "growth_plan"; st.rerun()
        elif growth_plan: st.warning("Previous plan generation failed.")
        if st.button("💡 " + ("Regenerate" if growth_plan else "Create") + " Plan", key="regen_create_plan_gem", use_container_width=True, type=("secondary" if growth_plan else "primary"), disabled=not analysis_successful):
            with st.spinner("✨ Working on plan with Gemini..."): plan = generate_growth_plan(analysis, {}); save_growth_plan(user_id, emotion_id, plan)
            if isinstance(plan, dict) and 'error' not in plan and 'raw_response' not in plan: st.session_state.current_view = "growth_plan"
            st.rerun()
        if not analysis_successful: st.caption("Analysis must succeed first.")
    with col2: # Resources Actions
        st.markdown("##### **AI-Generated Resources**"); res_exist = isinstance(resources, dict) and 'error' not in resources and 'raw_response' not in resources
        if res_exist: st.success("Personalized resources generated.")
        if st.button("📚 View Resources", key="view_res_btn_gem", use_container_width=True, disabled=not res_exist): st.session_state.current_view = "view_resources"; st.rerun()
        elif resources: st.warning("Previous resource generation failed.")
        if st.button("✨ " + ("Regenerate" if resources else "Generate") + " Resources", key="regen_create_res_gem", use_container_width=True, type=("secondary" if resources else "primary"), disabled=not analysis_successful):
            with st.spinner("✨ Generating resources with Gemini..."): res = generate_resources_from_profile(analysis, growth_plan); save_resource(user_id, emotion_id, res)
            if isinstance(res, dict) and 'error' not in res and 'raw_response' not in res: st.session_state.current_view = "view_resources"
            st.rerun()
        if not analysis_successful: st.caption("Analysis must succeed first.")

def render_growth_plan(emotion_id: str):
    st.title("🚀 Your Growth Plan"); user_id = st.session_state.current_user; plan = get_growth_plan(user_id, emotion_id); emotion_data = get_emotion_entry(user_id, emotion_id)
    if not plan: st.error("Growth plan not found.");
    if st.button("Back", key="back_plan_err_gem"): st.session_state.current_view = "emotion_analysis"; st.rerun(); return
    if emotion_data and isinstance(emotion_data.get('analysis'), dict): p_emotion = emotion_data['analysis'].get('primary_emotion', 'the situation'); st.caption(f"Plan related to your entry about: **{p_emotion}**")
    st.divider()
    if isinstance(plan, dict) and 'error' not in plan and 'raw_response' not in plan:
        st.header(plan.get('title', 'Personal Growth Plan')); steps = plan.get('steps', [])
    ...
    st.divider()
    if st.button("⬅️ Back to Analysis", key="back_plan_gem"): st.session_state.current_view = "emotion_analysis"; st.rerun()

def render_view_resources(emotion_id: str):
    st.title("📚 AI-Generated Resources"); user_id = st.session_state.current_user; resources = get_resources(user_id, emotion_id); emotion_data = get_emotion_entry(user_id, emotion_id)
    if not resources: st.error("Resources not found for this entry.");
    if st.button("Back", key="back_res_err_gem"): st.session_state.current_view = "emotion_analysis"; st.rerun(); return
    if emotion_data and isinstance(emotion_data.get('analysis'), dict): p_emotion = emotion_data['analysis'].get('primary_emotion', 'this situation'); st.caption(f"Resources generated based on your entry about: **{p_emotion}**")
    st.divider()
    if isinstance(resources, dict) and 'error' not in resources and 'raw_response' not in resources:
        ...
        if readings: st.subheader("📖 Recommended Reading");
    ...
    st.divider()
    if st.button("⬅️ Back to Analysis", key="back_res_gem"): st.session_state.current_view = "emotion_analysis"; st.rerun()


# --- Main Application Logic ---
def main():
    """Runs the Streamlit application."""
    # Page config is now at the top of the script
